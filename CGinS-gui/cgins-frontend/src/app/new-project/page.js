...
export default function NewProject() {
    ...
    const handleCreate = async () => {
        ...
        if (!formData.exampleInputsFile) {
            toast.error("example_inputs.pt is required");
            return;
        }
        if (formData.mode === "B" && !formData.fullModelFile) {
            toast.error("full_model.pt is required for Mode B");
            return;
        }
        ...
        if (formData.exampleInputsFile) {
            data.append("exampleInputsFile", formData.exampleInputsFile);
        }
        if (formData.fullModelFile) {
            data.append("fullModelFile", formData.fullModelFile);
        }
                ...
                <div className="p-8 space-y-8 max-w-xl mx-auto w-full mt-10">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                type="button"
                                onClick={() => handleInputChange("mode", "A")}
                                className={`h-12 rounded-lg border ${formData.mode === "A" ? "border-zinc-400 bg-zinc-900" : "border-zinc-800 bg-zinc-900/40"} text-zinc-200 text-sm`}
                            >
                                A - code + weights
                            </button>
                            <button
                                type="button"
                                onClick={() => handleInputChange("mode", "B")}
                                className={`h-12 rounded-lg border ${formData.mode === "B" ? "border-zinc-400 bg-zinc-900" : "border-zinc-800 bg-zinc-900/40"} text-zinc-200 text-sm`}
                            >
                                B - full model
                            </button>
                        </div>
                    ...
                    {formData.mode === "B" && (
                        <div className="space-y-2">
                            <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                                Upload Full Model (full_model.pt)
                            </label>
                            <div className="relative group">
                                <input
                                    type="file"
                                    accept=".pt"
                                    onChange={(e) => handleInputChange("fullModelFile", e.target.files[0])}
                                    className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                />
                                <div className="w-full h-20 bg-zinc-900/50 border-2 border-dashed border-zinc-800 group-hover:border-zinc-600 rounded-xl flex flex-col items-center justify-center transition-all">
                                    <p className="text-sm text-zinc-400 font-medium">
                                        {formData.fullModelFile ? formData.fullModelFile.name : "Click to select full_model.pt"}
                                    </p>
                                    <p className="text-xs text-zinc-600 mt-1">Required for Mode B</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                            Example Inputs (example_inputs.pt)
                        </label>
                        <div className="relative group">
                            <input
                                type="file"
                                accept=".pt"
                                onChange={(e) => handleInputChange("exampleInputsFile", e.target.files[0])}
                                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                            />
                            <div className="w-full h-20 bg-zinc-900/50 border-2 border-dashed border-zinc-800 group-hover:border-zinc-600 rounded-xl flex flex-col items-center justify-center transition-all">
                                <p className="text-sm text-zinc-400 font-medium">
                                    {formData.exampleInputsFile ? formData.exampleInputsFile.name : "Click to select example_inputs.pt"}
                                </p>
                                <p className="text-xs text-zinc-600 mt-1">Required by Jac backend</p>
                            </div>
                        </div>
                    </div>
