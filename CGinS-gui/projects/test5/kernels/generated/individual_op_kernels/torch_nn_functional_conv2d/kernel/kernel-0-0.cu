...
__global__ void conv2d_nchw_forward_kernel(
    ...
    const scalar_t* __restrict__ b, // nullable if bias_ptr==nullptr handled by passing nullptr
  ...
  int64_t in_c_start = g * in_c_per_g;
...
torch::Tensor launch(
  ...
  // arg0: input (N, C_in, H, W) expected contiguous NCHW
  // arg1: weight (C_out, C_in/groups, 3, 3) (K_h,K_w can be dynamic but profiler suggests 3x3)
  // arg2: bias (C_out) (may be undefined/empty in general, but provided here as Tensor)
  // arg3: stride (2)
  // arg4: padding (2)
  // arg5: dilation (2)
  // arg6: groups
  ...
  TORCH_CHECK(arg1.is_contiguous(), "weight must be contiguous");
  ...
  if (has_bias) {
    ...
    TORCH_CHECK(arg2.is_contiguous(), "bias must be contiguous");
  ...
  TORCH_CHECK(C_in_per_g * arg6 == C_in, "weight C_in/groups dimension mismatch with input and groups");
  ...
  // Output spatial size (floor division as in PyTorch conv2d)
