...
def create_and_validate(llm: GenModel, msg: str, model: str, provider: str, paths: dict[Path]) -> Tuple[str, bool, str]:
    ...
    """
    response = llm.chat(msg, model, provider=provider)
...
def generate(
    ...
    *,
    ...
    feedback, is_valid, error = create_and_validate(llm, msg, model, provider, paths)
    if is_valid:
    ...
    for i in range(3):
        print(f"\t\t\tReattempt {i}")
        _, is_valid, error = create_and_validate(llm, error, model, provider, paths)
        if is_valid:
            return feedback, True
