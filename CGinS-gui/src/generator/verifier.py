...
from src.llm_tools import complete
...
def summarize_issue_with_traceback(
    ...
    """
    sys_prompt = (
        "You analyze CUDA kernel compilation/runtime errors and provide actionable fixes.\n"
        "Important:\n"
        "- The caller-side argument formatting is already correct and MUST NOT be modified.\n"
        "- The issue will always be internal to the CUDA kernel code (argument order, typing,\n"
        "  indexing, launch signature, or parameter handling).\n"
        ...
        "- output or correct-output: Expected output tensor\n"
        "The CUDA kernel's launch() function MUST accept arguments in exactly this order:\n"
        f"{', '.join(input_and_output.get('signature', {}).get('params', ['arg0', 'arg1', '...']))}\n"
        "Provide specific recommendations for:\n"
        "1. Correct argument ordering and types in the kernel launch() signature\n"
        "2. Correct parameter use inside the CUDA code\n"
        "3. Indexing, pointer arithmetic, and shape-related issues\n"
    ...
    try:
        input_blob = json.dumps(input_and_output, default=str)
    except Exception:
        input_blob = str(input_and_output)
    ...
    return complete(sys_prompt, user_prompt)
...
def validate_kernel(
    ...
    try:
        # Make sure the current python env's bin dir is on PATH (ninja, etc.)
        # Do NOT hardcode CUDA_HOME; respect the user's CUDA setup (WSL varies).
        import sys
        python_bin_dir = os.path.dirname(sys.executable)
        os.environ["PATH"] = f"{python_bin_dir}:{os.environ.get('PATH','')}"

        # Best-effort CUDA_HOME discovery only if not already set.
        if "CUDA_HOME" not in os.environ:
            for cand in ("/usr/local/cuda", "/usr/local/cuda-12.1", "/usr/local/cuda-12.2", "/usr/local/cuda-12.0"):
